...
VIDEO_CARDS="${RADEON_CARDS}
	d3d12 freedreno intel lavapipe lima tegra nvk panfrost v3d vc4 virgl
	vivante vmware zink"
...
REQUIRED_USE="
	...
	video_cards_nvk? ( vulkan video_cards_tegra )
	vdpau? ( X )
	xa? ( X )
"
...
RDEPEND="
	...
	${LIBDRM_DEPSTRING}[video_cards_freedreno?,video_cards_intel?,video_cards_tegra?,video_cards_vc4?,video_cards_vivante?,video_cards_vmware?,${MULTILIB_USEDEP}]
	X? (
...
pkg_pretend() {
	if use vulkan; then
		if ! use video_cards_d3d12 &&
		   ...
		   ! use video_cards_virgl; then
			ewarn "Ignoring USE=vulkan     since VIDEO_CARDS does not contain d3d12, freedreno, intel, lavapipe, tegra, nvk, panfrost, radeonsi, v3d, or virgl"
		fi
	fi
	...
	if use vaapi; then
		if ! use video_cards_d3d12 &&
		   ...
		   ! use video_cards_radeonsi &&
		   ! use video_cards_virgl; then
			ewarn "Ignoring USE=vaapi      since VIDEO_CARDS does not contain d3d12, tegra, r600, radeonsi, or virgl"
		fi
	fi
	...
	if use vdpau; then
		if ! use video_cards_d3d12 &&
		   ...
		   ! use video_cards_radeonsi &&
		   ! use video_cards_virgl; then
			ewarn "Ignoring USE=vdpau      since VIDEO_CARDS does not contain d3d12, tegra, r600, radeonsi, or virgl"
		fi
	fi
	...
	if use xa; then
		if ! use video_cards_freedreno &&
		   ...
		   ! use video_cards_vmware; then
			ewarn "Ignoring USE=xa         since VIDEO_CARDS does not contain freedreno, intel, tegra, or vmware"
		fi
	fi
...
multilib_src_configure() {
	...
	if use vulkan; then
		...
		if use video_cards_nvk; then
			vulkan_enable video_cards_nvk tegra
